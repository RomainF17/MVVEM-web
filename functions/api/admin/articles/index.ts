...
export const onRequestPost: PagesFunction<Env> = async ({ env, request }) => {
  try {
    ...
    await env.DB.prepare(
      `INSERT INTO articles (id, title, summary, category, tags, coverImageUrl, contentMarkdown, address, status, authorEmail, publishedAt, updatedAt)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
    ).bind(
      id,
      body.title,
    ...
    return jsonResponse({ id, message: 'Article créé' }, 201);
  } catch (error) {
